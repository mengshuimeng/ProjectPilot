...
- 项目名称识别和已知别名归一化。
- 字段专属候选池和展示摘要。
- 轻量 lexical + semantic 混合 evidence 检索。
...
- 6 个 Agent Skills 管理项目 schema、写作、来源优先级、Demo 评分点、质量护栏和仓库说明规则。
- 句子级 claim-support 对齐和支撑明细。
- stdio MCP server 工作流工具。
- OCR fallback 接口预留。
- 回归测试覆盖核心链路。
...
- `doc / ppt` 老格式依赖本地转换工具。
- OCR fallback 当前仍是预留接口，未默认开启，也未接入更强的版面识别链路。
- evidence 检索已升级为轻量混合索引，但仍不是重型向量数据库。
- unsupported claims 和 claim-support 对齐不等同于完整学术查重或人工事实审查。
- 已提供 stdio MCP server；当前仍以本地工具链为主，未默认接入外部知识图谱或云端检索服务。
- 复杂 PDF 的章节、表格、指标行仍可能需要继续加强清洗规则。

## 后续规划

- 接入更稳定的 OCR fallback。
- 继续收紧项目名称抽取，减少学校、作者和赛事信息干扰。
- 为 UI 增加更强的 evidence 可视化和录屏提示。
- 评估是否引入更强的语义索引方案。
