...
def _load_processed_documents(project_root: Path) -> list[dict[str, Any]]:
    ...
    raw_docs = load_documents(project_root / "data" / "raw")
    return prepare_documents(raw_docs)
...
def run_generate(project_root: Path, output_type: str) -> dict[str, Any]:
    ...
    artifact = generate_artifact(profile, output_type, evidence=evidence, project_root=project_root)
...
def run_all(project_root: Path) -> dict[str, Any]:
    ...
    generated = {}
    for output_type in ("intro", "innovation", "defense"):
