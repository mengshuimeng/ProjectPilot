...
- `app/tool_registry.py`：提供本地工具注册层，当前包含文件检索和 Office 转换工具。
- `app/mcp_server.py`：提供 stdio MCP server，可将本地文件检索、raw 材料检索和 Office 转换暴露给支持 MCP 的客户端。
- `main.py`：提供 CLI 工作流。
...
- 当前已提供 stdio MCP server，但尚未接入外部知识图谱或云端检索服务。
