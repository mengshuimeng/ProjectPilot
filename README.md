...
ProjectPilot V2 ：通用 Harness + LLM 项目材料理解与答辩生成助手。它是一个面向任意项目材料的 AI 原生工具。它不再只服务某一个大创、算法、Web 或论文项目，而是让用户上传一个主材料文件，再可选上传补充材料，系统自动完成项目理解、证据检索、内容生成、校验和修复。

它不是普通聊天机器人，而是一个 Retrieval-Grounded + Harness-Controlled 的项目交付工作流。

代码仓库：<https://github.com/mengshuimeng/ProjectPilot>

本项目开发过程中使用 Codex 参与需求拆解、代码实现、UI 重构、测试补充和文档整理，符合 AI IDE 参与开发要求。

## 项目痛点

在课程项目、大创项目、科研项目或毕业设计中，项目材料往往分散在申报书、论文、README、PPT 提纲、测试说明和临时备注里。

这会带来几个非常真实的问题：

- 材料分散：写 README 或答辩稿时，需要反复翻很多文件，整理效率低。
- 版本冲突：不同文档口径不一致，容易把旧版本信息或错误内容混进最终输出。
- 脏文本污染：论文页眉页脚、目录、致谢、章节号、公式残片等内容，容易被误带进生成结果。
- 人工总结成本高：背景、痛点、创新点、交付物和局限性经常需要手动从多份材料中摘取、压缩和改写。
- 来源不可追踪：整理后的答辩稿或 README 往往只剩成稿，很难快速说明“这句话来自哪份材料”。

ProjectPilot 就是围绕这个痛点设计的。它不是开放式问答工具，而是把“材料解析、项目画像抽取、证据检索、内容生成、自动校验和修复”组织成一个可追踪、可校验的 Harness 工作流。
...
- 6 个 Agent Skills：管理项目 schema、写作规则、来源优先级、Demo 评分点、质量护栏和仓库说明规则。
- Anchor 优先证据检索：生成任务会优先检索 anchor 证据，再使用 supporting 补充，并结合轻量 lexical + semantic 混合索引做跨文档匹配。
- LLM 生成：支持 OpenAI-compatible API，包括 DeepSeek 等兼容服务。
- 规则 fallback：没有 API key、LLM 关闭或调用失败时仍可本地生成。
- 自动校验：检查 anchor 存在性、来源角色一致性、脏文本、重复表达、来源覆盖、unsupported claims 和句子级 claim-support 对齐。
...
- `skills/demo_rubric.md`：课程 Demo 与答辩评分点约束。
- `skills/quality_guardrails.md`：脏文本、字段重复、事实支撑和来源质量护栏。
- `skills/repository_rules.md`：README、仓库说明和提交边界规则。
...
- `app/tool_registry.py`：提供本地工具注册层，当前包含文件检索、Office 转换和本地知识图谱摘要工具。
- `app/mcp_server.py`：提供 stdio MCP server，可将本地文件检索、raw 材料检索、项目画像抽取、任务证据检索、校验、知识图谱摘要和工作流编排暴露给支持 MCP 的客户端。
- `main.py`：提供 CLI 工作流。
...
- `claim_evidence_alignment`：对生成输出中的关键声明做句子级 claim-support 对齐，结合关键词重叠和轻量语义相似度返回支撑明细。
...
ProjectPilot/
  app/
    ...
    tool_registry.py   # 本地工具注册层
    mcp_server.py      # stdio MCP server 入口
...
该 server 使用 stdio 传输，面向 MCP 客户端暴露以下工具：

- `projectpilot_status`：查看工作区、LLM 和工具状态。
- `search_project_files`：在 ProjectPilot 工作区检索文件。
- `search_raw_materials`：在 `data/raw` 中检索上传材料。
- `convert_office_material`：调用本地 LibreOffice 转换旧版 Office 文件。
- `extract_project_profile`：抽取并返回紧凑项目画像。
- `retrieve_task_evidence_mcp`：按任务返回 anchor 优先、语义增强的 evidence。
- `verify_project_materials`：返回当前工作区的校验报告。
- `project_knowledge_map`：输出本地项目知识图谱摘要。
- `orchestrate_project_task`：一次性编排抽取、检索、校验和可选生成流程。
